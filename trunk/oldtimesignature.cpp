...
bool OldTimeSignature::ConstructTimeSignature(TimeSignature& timeSignature) const
{
	...
	wxByte beatAmount = (wxByte)pow(2, (byTemp & 0x07));			    // Bottom 3 bits = ba in power of 2
...
bool OldTimeSignature::DoDeserialize(PowerTabInputStream& stream, wxWord version)
{
    //------Last Checked------//
    // - Dec 29, 2004
    WXUNUSED(version);
