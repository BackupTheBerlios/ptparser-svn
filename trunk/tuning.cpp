...
bool Tuning::DoDeserialize(PowerTabInputStream& stream, wxWord version)
{
    //------Last Checked------//
    // - Dec 14, 2004
...
wxSize Tuning::GetNoteRange(wxByte capo) const
{
    ...
    for (; i < stringCount; i++)
    {
        if (m_noteArray[i] != notUsed)
        {
            returnValue.x = min(returnValue.x, m_noteArray[i] + capo);
            returnValue.y = max(returnValue.y, m_noteArray[i] + capo + 24);       // Assume guitar has 24 frets
        }
    }
    return (returnValue);
}
