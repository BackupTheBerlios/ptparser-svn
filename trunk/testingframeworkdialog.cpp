...
TestingFrameworkDialog::TestingFrameworkDialog() :
    m_controlsInitialized(false), m_testingFramework(NULL), m_running(false), m_cancelTestRun(false), m_testsRun(0), 
    m_testsFailed(0), m_startTime(0), m_totalTime(0), m_testSuiteCheckListBox(NULL), m_resultsTreeCtrl(NULL),
    m_testSuiteStaticText(NULL), m_totalProgressGauge(NULL), m_testSuiteProgressGauge(NULL), m_testsRunTextCtrl(NULL),
    m_testsFailedTextCtrl(NULL), m_runButton(NULL), m_selectAllButton(NULL), m_deselectAllButton(NULL), 
    m_stopButton(NULL), m_exportButton(NULL), m_clearButton(NULL), m_OKButton(NULL)
{
    //------Last Checked------//
    // - Dec 13, 2004
    m_this = this;
    
    bool loaded = wxXmlResource::Get()->LoadDialog(this, NULL, wxT("IDD_TESTINGFRAMEWORK_DIALOG"));
...
void TestingFrameworkDialog::OnInitDialog(wxInitDialogEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(event);
...
void TestingFrameworkDialog::OnUpdateTestSuiteCheckListBox(wxUpdateUIEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
...
void TestingFrameworkDialog::OnBnClickedRunButton(wxCommandEvent& event) 
{
    //------Last Verified------//
    // - Dec 2, 2004
    RunTestSuites();
...
void TestingFrameworkDialog::OnBnClickedSelectAllButton(wxCommandEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(event);
...
void TestingFrameworkDialog::OnBnClickedDeselectAllButton(wxCommandEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(event);
...
void TestingFrameworkDialog::OnBnClickedStopButton(wxCommandEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(event);
...
void TestingFrameworkDialog::OnBnClickedExportButton(wxCommandEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(event);
    ...
    if (fileDialog.ShowModal() == wxID_OK)
    {
        ...
        wxString text;
        text = wxString::Format(wxT("Total time       : %.3f seconds"), m_totalTime);
        file.AddLine(text);
        text = wxString::Format(wxT("Tests run        : %s"), m_testsRunTextCtrl->GetValue().c_str());
        file.AddLine(text);
        text = wxString::Format(wxT("Tests failed     : %s"), m_testsFailedTextCtrl->GetValue().c_str());
        file.AddLine(text);
        text = wxString::Format(wxT("Date             : %s"), wxGetLocaleShortDateTime(wxDateTime::Now()).c_str());
        file.AddLine(text);
        file.AddLine(wxT(""));
        ...
        {
            long cookie = wxNewId();
            wxTreeItemId testSuiteTreeItemId = m_resultsTreeCtrl->GetFirstChild(rootTreeItemId, cookie);
            for (; testSuiteTreeItemId.IsOk(); testSuiteTreeItemId = m_resultsTreeCtrl->GetNextSibling(testSuiteTreeItemId))
            {
                wxString strItemText = m_resultsTreeCtrl->GetItemText(testSuiteTreeItemId);
                file.AddLine(strItemText);
                
                long cookie2 = wxNewId();
                wxTreeItemId messageTreeItemId = m_resultsTreeCtrl->GetFirstChild(testSuiteTreeItemId, cookie2);
                for (; messageTreeItemId.IsOk(); messageTreeItemId = m_resultsTreeCtrl->GetNextSibling(messageTreeItemId))
                {
...
void TestingFrameworkDialog::OnBnClickedClearButton(wxCommandEvent& event)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(event);
...
void TestingFrameworkDialog::RunTestSuites()
{
    ...
    wxTreeItemId rootItemId = m_resultsTreeCtrl->AddRoot(wxT("Test Run Results..."));
    ...
    if (userAborted)
        wxMessageBox(wxT("Test run aborted by user."), wxTheApp->GetAppName(), wxICON_EXCLAMATION);
    else
        wxMessageBox(wxString::Format(wxT("Test run completed in %.3f seconds."), m_totalTime), wxTheApp->GetAppName(), wxICON_INFORMATION);
}
...
void TestingFrameworkDialog::StartTestSuite(TestSuite* testSuite)
{
    ...
    wxTreeItemId treeItemId = m_resultsTreeCtrl->AppendItem(m_resultsTreeCtrl->GetRootItem(), strName, RESULTSTREECTRL_IMAGE_TIMER, RESULTSTREECTRL_IMAGE_TIMER);
    ...
    if (treeItemId)
    {
        ...
        wxString text = wxString::Format(wxT("[%s] - Starting test suite."), wxGetLocaleShortDateTime(wxDateTime::Now()).c_str());
        m_resultsTreeCtrl->AppendItem(treeItemId, text, RESULTSTREECTRL_IMAGE_TIMERDOC, RESULTSTREECTRL_IMAGE_TIMERDOC);
        PumpEvents();
    }
}
...
bool TestingFrameworkDialog::TestSuiteCallback(TestSuite* testSuite, const wxChar* testName, bool success, const wxChar* fileName, size_t lineNumber, bool record, double executionTime, void* clientData)
{
    //------Last Verified------//
    // - Dec 2, 2004
    WXUNUSED(clientData);
    ...
    m_this->m_testSuiteProgressGauge->SetValue(m_this->m_testSuiteProgressGauge->GetValue() + 1);
    m_this->PumpEvents();
    ...
    if (treeItemId)
    {
        ...
        if (!success)
        {
            wxString text = wxString::Format(wxT("[%s] - Test '%s' failed. %s(%d)"), wxGetLocaleShortDateTime(wxDateTime::Now()).c_str(), testName, fileName, lineNumber);
            wxTreeItemId error = m_this->m_resultsTreeCtrl->AppendItem(treeItemId, text, RESULTSTREECTRL_IMAGE_TEST_FAILURE, RESULTSTREECTRL_IMAGE_TEST_FAILURE);
            m_this->m_resultsTreeCtrl->SetItemTextColour(error, wxColour(0x7f, 0, 0));
            m_this->PumpEvents();
        ...
        {
            wxString text = wxString::Format(wxT("[%s] - Test '%s' completed in %.3f seconds."), wxGetLocaleShortDateTime(wxDateTime::Now()).c_str(), testName, executionTime);
            wxTreeItemId ok = m_this->m_resultsTreeCtrl->AppendItem(treeItemId, text, RESULTSTREECTRL_IMAGE_TEST_INFORMATION, RESULTSTREECTRL_IMAGE_TEST_INFORMATION);
    ...
    m_this->m_testsRunTextCtrl->SetValue(wxString::Format(wxT("%d"), m_this->m_testsRun));
    m_this->PumpEvents();

    // If we had a failure, update the tests failed text ctrl
    if (!success)
    {
        m_this->m_testsFailedTextCtrl->SetValue(wxString::Format(wxT("%d"), m_this->m_testsFailed));
        m_this->PumpEvents();
    }

    // Everything has gone OK; return the results of the user triggered cancel flag
    // in case user pushed Stop button
...
{   
    ...
    if (treeItemId)
    {
        ...
        // CASE: If the tests run total doesn't match the expected test count, display an error
        if (testSuite->GetTestsRun() != testSuite->GetTestCount())
        {
            image = RESULTSTREECTRL_IMAGE_TESTSUITE_WARNING;
            wxString strTestCount = wxString::Format(wxT("%d"), testSuite->GetTestCount());
            wxString strTestsRun = wxString::Format(wxT("%d"), testSuite->GetTestsRun());

            wxString text = wxString::Format(wxT("[%s] - GetTestCount returns a different value than the actual test run count. (%s vs %s)"),
                wxGetLocaleShortDateTime(wxDateTime::Now()).c_str(),
                strTestCount.c_str(),
                strTestsRun.c_str());
            wxTreeItemId warning = m_resultsTreeCtrl->AppendItem(treeItemId, text, RESULTSTREECTRL_IMAGE_TEST_WARNING, RESULTSTREECTRL_IMAGE_TEST_WARNING);
            m_resultsTreeCtrl->SetItemTextColour(warning, wxColour(0x7f, 0x7f, 0));
        ...
        double suiteTime = (double)((::wxGetLocalTimeMillis() - m_startTime).ToLong()) / 1000.0f;
        wxString text = wxString::Format(wxT("[%s] - Test suite completed in %.3f seconds. Tests run: %d; Tests failed: %d"), wxGetLocaleShortDateTime(wxDateTime::Now()).c_str(), suiteTime, testSuite->GetTestsRun(), testSuite->GetFailed());
        m_resultsTreeCtrl->AppendItem(treeItemId, text, RESULTSTREECTRL_IMAGE_TIMERDOC, RESULTSTREECTRL_IMAGE_TIMERDOC);
        PumpEvents();
        
        m_resultsTreeCtrl->SetItemImage(treeItemId, image);
        m_resultsTreeCtrl->SetItemImage(treeItemId, image, wxTreeItemIcon_Selected);
...
bool TestingFrameworkDialog::InitializeDialogControls()
{
    ...
    m_testSuiteCheckListBox = XRCCTRL(*this, "IDC_TESTSUITE_CHECKLISTBOX", wxCheckListBox);
    ...
    m_testSuiteStaticText = XRCCTRL(*this, "IDC_TESTSUITE_STATIC", wxStaticText);
    ...
    m_testSuiteProgressGauge = XRCCTRL(*this, "IDC_TESTSUITE_PROGRESS", wxGauge);
...
/// Populates the test suite list box with the names of the test suites in the testing framework
...
void TestingFrameworkDialog::InitializeResultsTreeCtrlImageList()
{
    ...
    m_resultsTreeCtrlImageList.Add(wxIcon(clipboardwithcheck_16x16_xpm), maskColour);
    ...
    m_resultsTreeCtrlImageList.Add(wxIcon(yellowexclamationpoint_16x16_xpm), maskColour);
    m_resultsTreeCtrlImageList.Add(wxIcon(redexclamationpoint_16x16_xpm), maskColour);
    
    m_resultsTreeCtrl->SetImageList(&m_resultsTreeCtrlImageList);
}

/// Determines if there is at least one test suite checked in the test suite list box
...
/// Allows event processing to continue in the dialog box while the tests are running
